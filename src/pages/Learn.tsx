...
export function Learn() {
  return (
    <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6">
      ...
      <div className="sticky top-16 z-10 -mx-4 mb-10 mt-6 overflow-x-auto border-b border-ink/8 bg-base/90 px-4 py-3 backdrop-blur-xl sm:mx-0 sm:rounded-xl sm:border sm:px-3">
        <div className="flex w-max gap-1 sm:w-full sm:justify-between">
          {IN_PAGE_NAV.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className="focus-ring shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium text-ink/50 transition hover:bg-ink/[0.06] hover:text-ink"
            >
              {item.label}
            </a>
          ))}
      ...
      <div className="mt-16 flex flex-wrap items-center justify-center gap-2 text-xs text-ink/30">
        {OPERATIONS.map((op) => (
