...
const ROLE_STYLES: Record<string, string> = {
  ...
  viewer: 'bg-ink/[0.06] text-ink/55 border-ink/12',
}
  ...
  if (users.length === 0) {
    return (
      <div className="panel flex flex-col items-center gap-3 rounded-2xl px-6 py-16 text-center">
        <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-ink/[0.05] text-ink/35">
          <Users className="h-6 w-6" />
        </span>
        <p className="text-sm font-medium text-ink/60">No users match your search.</p>
      </div>
    )
  }
            ...
            <AnimatePresence>
              {users.map((user) => (
                <motion.tr
                  ...
                  className="group border-b border-ink/6 transition-colors last:border-b-0 hover:bg-ink/[0.02]"
                >
                  <td className="px-5 py-3 font-medium text-ink">{user.name}</td>
                  <td className="px-5 py-3 text-ink/55">{user.email}</td>
                  ...
                  <td className="px-5 py-3 text-ink/40">{formatDate(user.createdAt)}</td>
                  <td className="px-5 py-3">
                    <div className="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                      <button
                        onClick={() => onEdit(user)}
                        aria-label={`Edit ${user.name}`}
                        className="focus-ring rounded-lg p-1.5 text-ink/45 transition hover:bg-ink/10 hover:text-ink"
                      >
                      ...
                      <button
                        ...
                        className="focus-ring rounded-lg p-1.5 text-ink/45 transition hover:bg-rose-500/15 hover:text-rose-400"
                      >
