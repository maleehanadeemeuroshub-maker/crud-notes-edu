...
export function TagsPanel({ tagCounts, activeTag, onFilterByTag, onRenameTag }: TagsPanelProps) {
  ...
  return (
    <div className="panel rounded-xl p-3">
      <p className="mb-2 flex items-center gap-1.5 px-1 text-[11px] font-semibold uppercase tracking-wide text-ink/40">
      ...
      <div className="flex flex-wrap gap-1.5">
        {tagCounts.map(({ tag, count }) =>
          editingTag === tag ? (
            <div key={tag} className="flex items-center gap-1 rounded-md bg-ink/[0.06] px-1.5 py-1">
              <input
                autoFocus
                value={draftName}
                ...
                className="focus-ring w-24 rounded bg-transparent text-xs text-ink outline-none"
              />
              ...
              <button type="button" onClick={() => setEditingTag(null)} aria-label="Cancel" className="text-ink/40 hover:text-ink/70">
                <X className="h-3 w-3" />
              </button>
            </div>
          ) : (
            <div
              ...
              className={cn(
                ...
                activeTag === tag ? 'bg-indigo-400/15 text-indigo-300' : 'bg-ink/[0.04] text-ink/55 hover:bg-ink/[0.07]',
                busyTag === tag && 'opacity-50',
              )}
            >
              <button type="button" onClick={() => onFilterByTag(activeTag === tag ? '' : tag)} className="focus-ring">
                #{tag} <span className="text-ink/30">{count}</span>
              </button>
              <button
                type="button"
                onClick={() => {
                ...
                className="focus-ring opacity-0 transition group-hover:opacity-100 hover:text-ink"
              >
